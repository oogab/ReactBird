...
const upload = multer({
    ...
    //         const basename = path.basename(file.originalname, ext) // 우크
    //         done(null, basename + '_' + new Date().getTime() + ext) // 우크183321312.png
    //     }
    // }),
...
router.post('/', isLoggedIn, upload.none(), async (req, res, next) => {    // POST /post
    try {
        ...
        if (req.body.image) {
            if (Array.isArray(req.body.image)) {    // 이미지를 여러개 올리면 image: [우크.png, 호두.png]
                const images = await Promise.all(req.body.image.map((image) => Image.create({ src: image })))
                await post.addImages(images)
            } else {    // 이미지를 하나만 올리면 image: 우크.png
...
router.patch('/:postId/like', isLoggedIn, async (req, res, next) => {     // PATCH /post/1/like
    try {
        ...
        await post.addLikers(req.user.id) // 이것 대신에 SQL 날릴 수도 있다!
...
router.post('/:postId/retweet', isLoggedIn, async (req, res, next) => {     // PATCH /post/1/retweet
    try {
        const post = await Post.findOne({
            where: { id: req.params.postId },
            include: [{
